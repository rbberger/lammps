...
Python3::Python3(LAMMPS *lmp) : Pointers(lmp)
{
  pyMain = NULL;
...
void Python3::command(int narg, char **arg)
{
...
void Python3::invoke_function(int ifunc, char *result)
{
  PyObject *pValue;
  char *str;
  ...
  for (int i = 0; i < ninput; i++) {
    ...
    if (itype == INT) {
      if (pfuncs[ifunc].ivarflag[i]) {
        ...
        pValue = PyLong_FromLong(atoi(str));
      } else pValue = PyLong_FromLong(pfuncs[ifunc].ivalue[i]);
      ...
      } else pValue = PyUnicode_FromString(pfuncs[ifunc].svalue[i]);
    } else if (itype == PTR) {
      pValue = PyCapsule_New((void *)lmp, NULL, NULL);
    }
  ...
  if (pfuncs[ifunc].noutput) {
    ...
    if (otype == INT) {
      sprintf(result,"%ld",PyLong_AsLong(pValue));
    } else if (otype == DOUBLE) {
      sprintf(result,"%.15g",PyFloat_AsDouble(pValue));
    } else if (otype == STRING) {
      char *pystr = PyUnicode_AsUTF8(pValue);
...
int Python3::variable_match(char *name, char *varname, int numeric)
{
  int ifunc = find(name);
  if (ifunc < 0) return -1;
...
char *Python3::long_string(int ifunc)
{
...
int Python3::create_entry(char *name)
{
